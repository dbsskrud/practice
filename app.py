...
# 1, 2. 타이틀 변경 및 설명 문구 추가
st.title("🏙️ 서울 스타터: 서울시 자취 가이드")
st.markdown("""
본 웹페이지는 서울에서 처음 자취를 시작하는 분들을 위해 제작되었습니다.  
**월세, 물가, 교통, 인프라** 데이터를 종합 분석하여 사용자의 우선순위에 맞는 최적의 자치구를 추천합니다.  
왼쪽 설정창에서 조건을 입력하고, 지도에서 상세 정보를 확인해 보세요!
""")
...
with col_info:
    ...
    # 6. 종합 추천점수 반원 도넛(Gauge) 차트
    max_possible_score = w_rent + w_price + w_culture + w_park
    score_val = (row['total_score'] / max_possible_score) * 100 if max_possible_score > 0 else 0
    ...
    <div style="text-align: center; border: 1px solid #ddd; padding: 20px; border-radius: 10px;">
        <p>💰 <b>평균 월세:</b> {row['평균월세']}만원</p>
        <p>🌳 <b>공원 수:</b> {row['공원수']}개</p>
        <p>📚 <b>공공도서관:</b> {row['도서관수']}개</p>
        <p>🎨 <b>기타 문화공간:</b> {int(row['기타문화공간'])}개</p>
        <hr>
        <p>🚇 <b>지나가는 호선:</b> {row['지하철호선']}호선</p>
    </div>
