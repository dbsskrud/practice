...
with col_map:
    ...
    fig = px.choropleth_mapbox(
        ...
        mapbox_style="carto-positron", zoom=10, center={"lat": 37.565, "lon": 126.985}, opacity=0.7, height=600
    )
    fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
    
    # 🔥 오류 수정 부분: 기존 선택 상태를 초기화하여 Streamlit API 충돌 방지
...
with col_info:
    ...
    m1, m2 = st.columns(2)
    with m1:
        st.markdown(f"<div style='text-align: center; padding: 10px; border: 1px solid #ddd; border-radius: 5px;'><small>평균 월세</small><br><strong style='font-size: 1.5em; color: #FF4B4B;'>{row['평균월세']}만원</strong></div>", unsafe_allow_html=True)
    with m2:
        p_c = "#e74c3c" if row['물가비율'] > 0 else "#27ae60"
        st.markdown(f"<div style='text-align: center; padding: 10px; border: 1px solid #ddd; border-radius: 5px;'><small>물가 지수</small><br><strong style='font-size: 1.5em; color: {p_c};'>{abs(row['물가비율']):.1f}%</strong><br><small style='color: {p_c};'>평균 대비 {'높음' if row['물가비율'] > 0 else '낮음'}</small></div>", unsafe_allow_html=True)
    st.divider()
    st.subheader("🏗️ 인프라 리포트")
    st.write(f"📚 **도서관:** `{row['도서관수']}개` / 🌳 **녹지:** `{row['공원수']}개` / 🚇 **교통:** `{row['지하철호선']}호선`")
    categories = ['월세', '물가', '문화', '녹지']
    fig_radar = go.Figure()
    fig_radar.add_trace(go.Scatterpolar(r=[row['norm_평균월세'], row['norm_생활물가'], row['norm_전체문화공간'], row['norm_공원수']], theta=categories, fill='toself', line_color='#FF4B4B'))
